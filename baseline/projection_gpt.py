...
f_handler = logging.FileHandler('logs_correct_thres_gpt2-large/new_datasets.log')
...
def train_sent(dataloader, model_name, embed_model_dim, config):
    ...
    save_path = 'blmodels_gpt2-large/' + type + '_' + config['dataset'] + '_' + config['embed_model']
...
def eval_sent(dataloader, model_name, embed_model_dim, config):
    ...
    baseline_model, optimizer, criterion = init_baseline_model(config, embed_model_dim, type=type)
    model_path = 'blmodels_gpt2-large/' + type + '_' + config['dataset'] + '_' + config['embed_model']
...
def eval_label(pred_labels,ground_truth,input, config,type = 'NN'):
    if type == 'NN':
        ...
        save_path = 'logs_test_correct_thres_gpt2-large/' +'test_' +type + '_' + config['dataset'] + '_' + config['embed_model'] +'_threshold_'+str(str_threshold)+'.label'
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        ...
        pred = [[] for i in range(len(pred_labels))]
        gt = [[] for i in range(len(pred_labels))]
        save_path = 'logs_test_correct_thres_gpt2-large/' +'test_' + type + '_' + config['dataset'] + '_' + config['embed_model'] + '.label'
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str, default='baseline_weights/gpt2-large', help='Dir of your model')
