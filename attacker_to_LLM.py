...
def process_data(
    ...
    optimizer.add_param_group({"params": sentence_embedding_model.parameters()})
    ...
    for i in range(num_epochs):
        ...
        torch.save(
            sentence_embedding_model.state_dict(),
...
def process_data_test(dataset: LLM_dataset, config: dict):
    ...
    sentence_embedding_model.to(config["device"])
    ...
    with torch.no_grad():
        count = 0
        for idx, (batch_text, last_hidden_states) in enumerate(
            tqdm(dataloader, desc="Processing Batches")
        ):
...
if __name__ == "__main__":
    ...
    model_cards["meta-llama2-7b"] = "meta-llama/Meta-Llama2-7B"
